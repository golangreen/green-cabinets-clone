 ...
 */
async function recordMetric(metric: Omit<PerformanceMetric, 'id'>): Promise<void> {
  try {
 ...
 */
async function getSlowest(params: {
  startDate: Date;
  endDate: Date;
  limit?: number;
}): Promise<PerformanceMetric[]> {
 ...
 */
async function cleanupOldMetrics(daysToKeep: number = 30): Promise<number> {
  try {
...
/**
 * Performance Service - Object-based export
