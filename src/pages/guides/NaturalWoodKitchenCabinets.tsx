      ...
      <section className="py-16 sm:py-20 md:py-28">
        <div className="container mx-auto px-4 sm:px-6 max-w-3xl">
          ...
          <p className="text-base text-muted-foreground mb-12 leading-relaxed">
            ...
            <Link to={link("white-oak")} className="text-accent-foreground font-semibold hover:underline">white oak</Link>{" "}
            in plain or{" "}
            <Link to={link("rift-cut-white-oak")} className="text-accent-foreground font-semibold hover:underline">rift-cut</Link>,
            finished in hardwax oil. Want premium?{" "}
            <Link to={link("walnut")} className="text-accent-foreground font-semibold hover:underline">walnut</Link>.
            Want cheap?{" "}
            <Link to={link("ash")} className="text-accent-foreground font-semibold hover:underline">white ash</Link>.
            Want maximum durability?{" "}
            <Link to={link("hickory")} className="text-accent-foreground font-semibold hover:underline">hickory</Link>.
              ...
              >
                <div className="flex items-start justify-between gap-4 mb-2">
                  <h3 className="text-xl sm:text-2xl font-bold text-foreground">
                    <Link to={link(p.slug)} className="hover:text-accent-foreground transition-colors">
                ...
                <p className="text-sm italic text-accent-foreground mb-2">{p.look}</p>
                ...
                <p className="text-sm text-muted-foreground">
                  ...
                  <Link to={link(p.slug)} className="text-accent-foreground hover:underline">Full {p.name} spec sheet →</Link>
                </p>
              </article>
            ))}
          </div>
          ...
          <ul className="list-disc pl-5 space-y-2 text-base text-muted-foreground mb-12 leading-relaxed">
            <li>
              <Link to={link("red-oak")} className="text-accent-foreground hover:underline">Red oak</Link> — pink-orange undertone reads dated unstained. Stain it and it's a budget hero.
            </li>
            <li>
              <Link to={link("birch")} className="text-accent-foreground hover:underline">Birch</Link> — color varies wildly door to door, looks restless without a unifying stain.
            </li>
            <li>
              <Link to={link("alder")} className="text-accent-foreground hover:underline">Knotty alder</Link> — too soft for natural; the knots crack and the surface dents. Great for distressed-rustic stained doors.
            </li>
            <li>
              <Link to={link("beech")} className="text-accent-foreground hover:underline">European beech</Link> — has a pinkish cast that fights with most stones and backsplashes when left clear.
            </li>
          </ul>
              ...
              return (
                <div key={f.name} className="border border-border rounded-lg p-5">
                  <h3 className="font-bold text-foreground mb-2 flex items-center gap-2">
                    <Icon className="h-5 w-5 text-accent-foreground" /> {f.name}
                  </h3>
          ...
          <ul className="list-disc pl-5 space-y-3 text-base text-muted-foreground mb-12 leading-relaxed">
            <li>
              ...
              <Link to={link("white-oak")} className="text-accent-foreground hover:underline">plain white oak</Link>.
            </li>
            <li>
              <strong>Rift-sawn:</strong> Straight, parallel grain lines. ~25–35% pricier. The most modern read — see{" "}
              <Link to={link("rift-cut-white-oak")} className="text-accent-foreground hover:underline">rift-cut white oak</Link>.
            </li>
            <li>
              <strong>Quartersawn:</strong> Straight grain with visible medullary ray flecks. Mission/Craftsman classic — see{" "}
              <Link to={link("quartersawn-oak")} className="text-accent-foreground hover:underline">quartersawn oak</Link>.
            </li>
          </ul>
          ...
          <ul className="list-disc pl-5 space-y-2 text-base text-muted-foreground">
            <li><Link to="/best-wood-for-kitchen-cabinets" className="text-accent-foreground hover:underline">Best wood for kitchen cabinets — picks by use case</Link></li>
            <li><Link to="/cabinet-wood-types-and-costs" className="text-accent-foreground hover:underline">Cabinet wood types and costs — all 14 species compared</Link></li>
            <li><Link to="/finishes-colors" className="text-accent-foreground hover:underline">Finishes & colors — see natural-wood swatches in person</Link></li>
          </ul>
        </div>
      </section>
    </main>
